...
The repository-root `AGENTS.md` and `CLAUDE.md` make this standard mandatory
for their respective agent runtimes; `README.md` and
...
- Aggregate all phase-owned files for a multi-file phase such as Phase 11.
...
with a component-status table. For UI work, use the exact format below:

| Component or surface | Functional status | Clean-rebrand status |
|---|---|---|
| Named component | What behavior is implemented and verified | What visual work is complete or remains under Phase 20 |

For non-UI work, preserve the same intent with these columns:
...
Component rows must not collapse distinct states. For example, an Astryx-
migrated Navbar can be `Functionally complete` while its clean visual rebrand
...
For a repository-wide UI report, use `Component or surface | Functional
status | Clean-rebrand status` for the second table, matching the Phase 20
separation between behavior and visual acceptance.
